...
def process(input_file):
    ...
    # Encode: [f, (x, y, direction, v, step, cost)]
    queue.append([0, (start[0], start[1], 0, 0, 0, 0)])

    visited = set([])
    count_nodes = 0
    # heuristic = manhattan / vmax
    while (len(queue) > 0):
        queue = sorted(queue)
        ...
        if ((x, y, d, v) not in visited):
            ...
            if v == 0:
                ...
                queue.append(
                    [p[0] - 1, (x, y, turn_left(d), v, step + 1, cost + 1)])

                queue.append(
                    [p[0] - 1, (x, y, turn_right(d), v, step + 1, cost + 1)])

                x3, y3 = next_position(x, y, d, v+1, A, N)
                if is_inside(x3, y3, N):
                    heuristic = float(
                        abs(x3 - goal[0]) + abs(y3 - goal[1])) / (vmax)
                    queue.append([(cost + 1 + math.sqrt(v+1) + heuristic) * -1, (x3, y3, d, v+1, step + 1, cost + 1 + math.sqrt(v+1)
                                                                                 )])
                ...
                if is_inside(x1, y1, N):
                    heuristic = float(
                        abs(x1 - goal[0]) + abs(y1 - goal[1])) / (vmax)
                    queue.append([(cost + 1 + math.sqrt(v) + heuristic) * -1,
                                  (x1, y1, d, v, step + 1, cost + 1 + math.sqrt(v))])
                ...
                if v > 0:
                    ...
                    if is_inside(x2, y2, N):
                        ...
                        heuristic = float(
                            abs(x2 - goal[0]) + abs(y2 - goal[1])) / (vmax)
                        queue.append([(cost + 1 + math.sqrt(v-1) + heuristic) * -1, (x2, y2, d, v-1, step + 1, cost + 1 + math.sqrt(v-1)
                                                                                     )])
